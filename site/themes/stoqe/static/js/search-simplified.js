...
var aptsHits = instantsearch.widgets.infiniteHits( {
  ...
  empty: '',
  templates: {
    item: '<span class="db mv3"><strong><span class="ttu has-text-primary">{{{_highlightResult.SiteName.value}}}</span></strong><br><i class="mr1 has-text-grey-light fas fa-map-marker-alt"></i> {{{_highlightResult.ShopLevel.value}}} {{{_highlightResult.Building.value}}} {{{_highlightResult.StreetNumber.value}}} {{{_highlightResult.StreetAddress.value}}}, {{{_highlightResult.Suburb.value}}}, {{{_highlightResult.State.value}}}, {{{_highlightResult.Postcode.value}}}</span><hr class="mv0">',
    empty: getTemplate( 'no-results' )
  }
} )
